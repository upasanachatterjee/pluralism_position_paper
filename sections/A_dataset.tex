...
\paragraph{Expertise.} Expertise perturbations are assigned based on the category of the question. Given the pool of expertise available for each category, we first take the hash of the evidence passage modulo the size of the pool to select an expert identity. This ensures that the same passage always corresponds to the same expert, while still varying the expert across passages. The full list of categories and their corresponding expertise is provided in Table~\ref{tab:expertise}.
...
\textbf{Casual Register Instruction}
...
We use the ConflictingQA dataset \citep{wan-etal-2024-evidence} for our experiments. The original dataset contains 419 questions, from which we manually select 90: 45 epistemic and 45 aleatoric. Tables~\ref{tab:epistemic_questions} and~\ref{tab:aleatoric_questions} list these questions with their categories. Table~\ref{tab:category-axis} shows the number of questions per category for each question type. We note that the epistemic questions are more heavily concentrated in the medicine and life sciences categories, while the aleatoric questions are more concentrated in the philosophy and religion categories. This is a natural consequence of the question types: epistemic questions are more likely to be grounded in scientific evidence, while aleatoric questions are more likely to be grounded in social and cultural norms.
